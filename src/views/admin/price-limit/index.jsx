...
import {limit_price, limit_price_save_or_edit, user_merchant_search,} from '../../../contexts/api';
...
const PriceLimit = () => {
    ...
    const getLimitUrl = () => {
        ...
        return `${limit_price}?page=${page}&size=${size}${queryParams ? `&${queryParams}` : ''}`;
    }
    ...
    const getFunctionUserSearch = async () => {
        await globalGetFunction({
            url: `${user_merchant_search}${merchantSearch ? !isNaN(+merchantSearch) ? `?phone=${merchantSearch}` : `?fullName=${merchantSearch}` : ''}`,
    ...
    const handleSave = () => {
        ...
        Object.keys(formValues).forEach((key) => {
            if (formValues[key]?.trim() === '') errors[key] = `${key} ${wordsListData?.IS_REQUIRED}`;
        });
        if (Object.keys(errors).length === 0) {
            if (!formValues.userId) return toast.error(wordsListData?.ERROR_MERCHANT || 'Выбор продавца обязателен')
            if (+formValues.min < 1000 || +formValues.min > 150000000) return toast.error(wordsListData?.ERROR_LIMIT || 'Сумма должна быть не менее 1000 сум.')
            if (+formValues.max < 1000 || +formValues.max > 150000000) return toast.error(wordsListData?.ERROR_LIMIT || 'Сумма должна быть не менее 1000 сум.')
            if (+formValues.min >= +formValues.max) return toast.error(wordsListData?.ERROR_MIN_MAX || 'Минимальная сумма не может быть больше максимальной суммы')
            setLoading(true);
            globalPostFunction({
    ...
    return (
        <>
            <Box pt={{base: '130px', md: '80px', xl: '80px'}}>
                ...
                >
                    <Card flexDirection={{base: 'column', md: 'row'}} gap={'10px'} borderRadius={'15px'} w="100%"
                          p="10px">
                        <Box flex="1">
                            <Input
                        ...
                        limitPrice.object.length > 0 ? (
                            limitPrice.object.map((item, i) => (
                                <Tr key={item?.id || i}>
                                    ...
                                    <Td minWidth={'250px'}>
                                        {item?.phone
                                            ? `+998 (${item?.phone.slice(3, 5)}) ${item?.phone.slice(5, 8,
            ...
            >
                ...
                <ModalContent>
                    <ModalHeader>
                        {editStatus === 'EDIT' ? wordsListData?.LIMIT_EDIT || 'Редактирование лимита' : wordsListData?.LIMIT_CREATE || 'Создать лимит'}
                    </ModalHeader>
                    <ModalCloseButton/>
                    <ModalBody pb={6}>
                        <Grid templateColumns="repeat(2, 1fr)" gap={6} px={5}>
                            <FormControl mt={4} isInvalid={!!formErrors.userId}>
                                <FormLabel>{wordsListData?.MERCHANT_SELECT || 'Выберите продавца'}</FormLabel>
                                <Select
                                    name={'userId'}
                                    placeholder={wordsListData?.MERCHANT_SELECT || 'Выберите продавца'}
                                ...
                                {formErrors.userId && (
                                    <Text color="red.500" fontSize="sm">{formErrors.userId}</Text>
                                )}
                            </FormControl>
                            <FormControl mt={4}>
                                <FormLabel>{wordsListData?.MERCHANT_SEARCH || 'Поиск продавца'}</FormLabel>
                                <Input
                                    placeholder={wordsListData?.MERCHANT_SEARCH || 'Поиск продавца'}
                            ...
                            <FormControl mt={4} isInvalid={!!formErrors.min}>
                                <FormLabel>{wordsListData?.LIMIT_PRICE_MIN || 'Минимальная сумма'}</FormLabel>
                                <Input
                                    name="min"
                                    placeholder={wordsListData?.LIMIT_PRICE_MIN || 'Минимальная сумма'}
                                    value={formValues.min}
                                    onChange={e => {
                                        const val = e.target.value
                                        if (!isNaN(+val) && +val >= 0 && +val <= 150000000) handleChange(e)
                                    }}
                                    onKeyDown={e => {
                                ...
                                />
                                {formErrors.min && (
                                    <Text color="red.500" fontSize="sm">{formErrors.min}</Text>
                                )}
                            </FormControl>
                            <FormControl mt={4} isInvalid={!!formErrors.max}>
                                <FormLabel>{wordsListData?.LIMIT_PRICE_MAX || 'Максимальная сумма'}</FormLabel>
                                <Input
                                    name="max"
                                    placeholder={wordsListData?.LIMIT_PRICE_MAX || 'Максимальная сумма'}
                                ...
                                />
                                {formErrors.max && (
                                    <Text color="red.500" fontSize="sm">{formErrors.max}</Text>
                                )}
                            </FormControl>
                        </Grid>
                    </ModalBody>
