...
const PriceLimit = () => {
  ...
  const getFunctionUserSearch = async () => {
    await globalGetFunction({
      ...
      setLoading: setMerchantSearchLoading
    });
  };
  ...
  useEffect(() => {
    ...
    if (!merchantSearch) formValues.userId = '';
  }, [merchantSearch]);
    ...
    <>
      <Box pt={{base: '130px', md: '80px', xl: '80px'}}>
        ...
        >
          ...
          >
            {loading ? (
            ...
            ) : limitPrice && limitPrice.object && limitPrice.object.length > 0 ? (
              limitPrice.object.map((item, i) => (
                <Tr key={item?.id || i}>
                  ...
                  <Td minWidth={'250px'}>
                    {item?.phone
                      ? `+998 (${item?.phone.slice(3, 5)}) ${item?.phone.slice(5, 8)} ${item?.phone.slice(8, 10)} ${item?.phone.slice(10)}`
                      : '-'
                    }
                  </Td>
      ...
      >
        ...
        <ModalContent>
          ...
          <ModalCloseButton/>
          <ModalBody pb={6}>
            <Grid templateColumns="repeat(1, 1fr)" px={5}>
              ...
              {/*    )}*/}
              {/*</FormControl>*/}
              <FormControl mt={4} position="relative" isInvalid={!!formErrors.userId}>
                <FormLabel>
                ...
                <Input
                  ...
                  onBlur={(e) => {
                    // Agar tanlash bo'lmasa va ma'lumot bo'sh bo'lsa, dropdownni yopish
                ...
                {showDropdown && merchantData?.length > 0 ? (
                  <Card
                    position="absolute"
                    top="100%"
                    left="0"
                    ...
                    <List>
                      {merchantSearchLoading ?
                        <Text textAlign={'center'}>
                          {`${wordsListData?.LOADING}...` || 'Loading...'}
                        </Text> : merchantData.map((item) => (
                          <Text
                            onPointerDown={() => {
                              setMerchantSearch(`${item.firstName} ${item.lastName}`);
                              handleChange({target: {name: "userId", value: item.id}});
                              setShowDropdown(false); // Dropdownni yopish
                            }}
                            key={item.id}
                            marginBottom="2px"
                            cursor="pointer"
                  ...
                  >
                    <Text textAlign="center">
                      {wordsListData?.NOT_FOUND || 'Not found'}
                    </Text>
                  </Card>
                ) : null}
              </FormControl>
